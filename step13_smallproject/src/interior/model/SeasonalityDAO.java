...
public class SeasonalityDAO {
	...
	public static SeasonalityDTO getSeasonality(int pnum) throws SQLException {
		...
		try {
			...
			if (rset.next()) {
				Seasonality = new SeasonalityDTO(rset.getInt(1), rset.getString(2), rset.getInt(3), rset.getString(4),
	...
	public static ArrayList<SeasonalityDTO> getAllSeasonality() throws SQLException {
		...
		try {
			...
			while (rset.next()) {
				list.add(new SeasonalityDTO(rset.getInt(1), rset.getString(2), rset.getInt(3), rset.getString(4),
						rset.getString(5), rset.getInt(6)));
			}
		} finally {
			DBUtil.close(con, pstmt, rset);
